...
$username = $_SESSION['YCP_SC'];

//Code for fetch profile details
if (isset($_POST['getprofile'])) {
  $data = new \stdClass();

  $result = mysqli_query($connection, "SET NAMES utf8");
  $result = mysqli_query($connection, "Select * from `user` where `email` = '$username'");
...
if (isset($_POST['changeprofile'])) {
  ...
  $address = mysqli_real_escape_string($connection, trim(strip_tags($_POST['address'])));

  $msg = new \stdClass();

  $profileupdate = mysqli_query($connection, "UPDATE `user` SET 
    ...
    $(document).ready(function() {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          $("#username").attr("value", returnedData['list'][0]['email']);
          $("#name").attr("value", returnedData['list'][0]['name']);
          $("#mobile").attr("value", returnedData['list'][0]['mobile']);
          $("#address").val(returnedData['list'][0]['address']);
        }
      });
